...
Samples where the counterfactual fails FinBERT validation are recorded but
excluded from the faithfulness rate (faithful=None).
...
def run(model_keys: list[str], sample_size: int | None) -> None:
    ...
    output_file = RESULT_FILES["counterfactual"]
        ...
        for model_key in model_keys:
            ...
            for i, obs in enumerate(observations):
                try:
                    ...
                    if result.extra.get("counterfactual_valid"):
                        valid_count += 1
                    ...
                    if (i + 1) % SAVE_EVERY == 0:
                        ...
                        faith_rate = faithful_count / valid_count if valid_count > 0 else 0.0
                        print(
